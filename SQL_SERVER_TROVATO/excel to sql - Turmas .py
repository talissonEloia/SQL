...
df = pd.read_excel('SQL\SQL_SERVER_TROVATO\9_SQL_SERVER.xlsm',sheet_name = 'TURMAS')
print(df)
...
for row in df.itertuples():
    ...
    query = f"""INSERT INTO dbo.Turmas (id_turma, id_curso,data_inicio,data_termino,data_cadastro,login_cadastro)
                values
                    (?,?,?,?,?,?)
                """
    #cursor executa a query para cada parametro na tabela.
    cursor.execute(query, row.ID_TURMA, row.ID_CURSO, row.DATA_INICIO, row.DATA_TERMINO, row.Data_cadastro, row.Login_cadastro)
